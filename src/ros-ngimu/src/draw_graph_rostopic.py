...
def Imucallback(msg):
    ...
    x_acc_list.append(msg.angular_velocity.x)
    y_acc_list.append(msg.angular_velocity.y)
    z_acc_list.append(msg.angular_velocity.z * 180 / 3.14)
...
def animate(i):
    ...
    if len(x_esti_list):
        plt.cla()
        plt.plot(x_index[int(len(z_acc_list)/3):len(z_acc_list)], z_acc_list[int(len(z_acc_list)/3):len(z_acc_list)])
